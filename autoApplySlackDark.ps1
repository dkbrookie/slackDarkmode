...
If($slackContent -like "*https://raw.githubusercontent.com/dkbrookie/slackDarkmode/master/darkmode.css*") {
  Write-Output "You already have the dark theme applied!"
} Else {
  Write-Output "
document.addEventListener('DOMContentLoaded', function () {
    $.ajax({
        url: 'https://raw.githubusercontent.com/dkbrookie/slackDarkmode/master/darkmode.css',
        success: function (css) {
            $("<style></style>").appendTo('head').html(css);
        }
    });
});" | Out-File $slackFile -Append -Encoding ASCII

  Stop-Process -Name slack -Force
